-- Fix: Remove duplicate user and update the correct record
-- Delete the duplicate user record (wrong ID)
DELETE FROM public.users WHERE email = 'wasperstore@gmail.com' AND id != '1debb8f1-ccfc-4edb-b187-0695eac42ae8';

-- Insert the correct user record with the right ID from auth.users
...
-- Fix the custom access token hook to handle RLS properly
