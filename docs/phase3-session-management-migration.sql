...
BEGIN
  -- Mark sessions as inactive if they've exceeded max idle time
  ...
  WHERE 
    is_active = true
    AND (
      -- Session exceeded max idle time
...
BEGIN
  -- Only super admins or the user themselves can revoke all sessions
...
AS $$
BEGIN
  -- Only update last_activity_at if heartbeat_count increased
...
COMMENT ON FUNCTION public.expire_stale_sessions() IS 'Auto-expires sessions exceeding max idle time';
